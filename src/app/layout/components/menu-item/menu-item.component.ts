...
export class MenuItemComponent implements OnInit{
  @Input() item: any;
    ...
    constructor(
        ...
        private sessionService: PersonasService
    ) {
    ...
    itemClick(event: MouseEvent) {
        ...
        this.redireccion(this.item.LINK);
    }

    redireccion(link: string) {
    if (link.trim() != "" && link.trim().toUpperCase() != "SALIR") {
      let redirectWe8Local;
      if (link.includes("RedirectWE8")) {
        redirectWe8Local = link.split("=")[1];
        this.router.navigate([redirectWe8Local]);
      }
      else if (link.startsWith("URL:")) {
        window.open(link.replace("URL:", ""), "_blank");
      } else if (link.toLowerCase().includes("aspx")) {
        window.location.href = server + '/WE7/' + link.replace("../", "").replace("..\\", "")
      }
      else {
        this.router.navigate([link]);
      }
    } else if (link.trim().toUpperCase() == "SALIR") {
